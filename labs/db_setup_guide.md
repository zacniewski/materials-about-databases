...
1. W drzewie po lewej stronie odszukaj swoją bazę danych (np. `s12345`).
1. Kliknij prawym przyciskiem myszy na nazwie swojej bazy i wybierz **Query Tool**.
1. W otwartym oknie edytora SQL możesz wczytać plik na dwa sposoby:
...
1. Kliknij ikonę błyskawicy lub naciśnij klawisz **F5**, aby wykonać skrypt.
1. Sprawdź komunikaty w zakładce *Messages* na dole – powinien pojawić się komunikat o pomyślnym wykonaniu zapytania (np. `Query returned successfully`).
...
1. Jeśli nie widzisz nowych tabel, kliknij prawym przyciskiem myszy na `Tables` i wybierz **Refresh**.
1. Możesz sprawdzić zawartość tabeli, wpisując w *Query Tool* polecenie:
   ```sql
